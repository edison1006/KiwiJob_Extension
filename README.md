...
All application code for this project lives under **`kiwijob/`**. Clone the repo, then `cd kiwijob` for installs, builds, and Docker Compose. **GitHub shows this file** (`README.md` at the repository root) on the repo home page.

KiwiJob is a **Chrome Extension** + **React dashboard** + **FastAPI API** for saving job postings, tracking application status, uploading a CV (PDF/DOCX), and running an **AI JD ↔ CV match** analysis.

Monorepo layout (under `kiwijob/`):

- `kiwijob/apps/web` — Vite + React + TypeScript + Tailwind dashboard
- `kiwijob/apps/extension` — Manifest V3 Chrome extension (content script + service worker + side panel)
- `kiwijob/apps/api` — FastAPI + SQLModel + PostgreSQL API
- `kiwijob/packages/shared` — Shared TypeScript contracts (`JobSavePayload`, statuses, DTO shapes)
...
```bash
psql -U postgres -d postgres -f kiwijob/scripts/postgres-init.sql
```

Then in `kiwijob/apps/api/.env` set:

`DATABASE_URL=postgresql+psycopg2://kiwijob:kiwijob@localhost:5432/kiwijob`
...
**Important:** run `uvicorn` only from `kiwijob/apps/api` (where the `app/` package lives). If you see `No module named 'app'`, you are in the wrong directory. From repo root you can use:

```bash
bash kiwijob/apps/api/dev.sh
```
...
```bash
cd kiwijob
npm run build -w @kiwijob/extension
```

Then in Chrome (**114+**, Side Panel API): **Extensions → Load unpacked →** select `kiwijob/apps/extension/dist`.

Notes:

- **Open the UI:** click the KiwiJob toolbar icon — the **side panel** opens (no popup). You can dock it on the left or right with Chrome’s side panel controls.
...
- **Toolbar icons:** `kiwijob/apps/extension/public/manifest.json` references PNGs in `kiwijob/apps/extension/public/icons/`. Regenerate with `npm run icons -w @kiwijob/extension` (or `python3 scripts/render_icons.py` from `kiwijob/apps/extension`) and rebuild.
- **Privacy policy (Chrome Web Store):** see [`kiwijob/docs/PRIVACY.md`](kiwijob/docs/PRIVACY.md). Use a public raw GitHub URL to that file on your default branch in the listing (pattern in [`kiwijob/docs/RELEASE.md`](kiwijob/docs/RELEASE.md)).
...
Postgres: `localhost:5432` (`kiwijob` / `kiwijob`)
...
- `POST /copilot/cover-letter` — generate or fall back to a profile-based cover letter
...
npm run build:web
npm run build:extension   # output: kiwijob/apps/extension/dist
```
