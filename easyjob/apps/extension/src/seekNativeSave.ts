...
/**
 * When the user clicks SEEK's native Save on a job ad, sync the listing to EasyJob (same as side panel Save job).
 */
  ...
  document.addEventListener(
    ...
    (ev: MouseEvent) => {
      ...
      try {
        ...
        showToast(`EasyJob: could not read job — ${(e as Error).message}`, false);
        return;
      }
