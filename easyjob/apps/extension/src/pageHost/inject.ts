...
import { EASYJOB_CLOSE_DRAWER, EASYJOB_EXTENSION_SOURCE } from "./messages";

const HOST_ID = "easyjob-chrome-page-host";
...
function ensureHost(): HTMLElement {
  ...
  host.id = HOST_ID;
  host.setAttribute("data-easyjob-host", "");
  ...
  launcher.title = "EasyJob";
  launcher.setAttribute("aria-label", "Open EasyJob");
  ...
  window.addEventListener("message", (ev: MessageEvent) => {
    if (ev.data?.type === EASYJOB_CLOSE_DRAWER && ev.data?.source === EASYJOB_EXTENSION_SOURCE) {
      closeDrawer();
    }
  });
...
export function initEasyJobPageHost(): void {
  const p = window.location.protocol;
  if (p !== "http:" && p !== "https:") return;
  ensureHost();
}

export function toggleEasyJobPageHost(): void {
  const p = window.location.protocol;
  if (p !== "http:" && p !== "https:") return;
