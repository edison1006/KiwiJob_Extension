...
import { APPLICATION_STATUSES, type ApplicationStatus } from "@easyjob/shared";
...
function exportApplicationsCsv(rows: ApplicationListItem[]) {
  ...
  a.href = url;
  a.download = `easyjob-applications-${new Date().toISOString().slice(0, 10)}.csv`;
  a.click();
...
export default function JobsPage() {
        ...
        <details className="relative sm:text-right">
          ...
          <div className="absolute right-0 z-10 mt-2 w-[min(100vw-2rem,22rem)] rounded-xl border border-slate-200 bg-white p-4 text-left text-sm text-slate-600 shadow-lg sm:w-80">
            ...
            <p className="mt-2 leading-relaxed">
              Install the <span className="font-semibold text-slate-800">EasyJob</span> Chrome extension, open a job page (e.g. SEEK{" "}
  ...
  const pageSubtitle = isMatchesRoute
    ? "Roles where EasyJob has stored a JD↔CV (or JD-only mock) match score."
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-slate-200 bg-slate-50/80 px-6 py-16 text-center">
          ...
          <p className="mt-2 max-w-md text-sm leading-relaxed text-slate-600">
            Use the EasyJob extension on a job posting, or connect the API and refresh. Everything you save shows up here with status and match
            tools.
          </p>
        </div>
      </div>
