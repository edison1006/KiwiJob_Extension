...
import { UserMenu, EASYJOB_PREFS_EVENT, readDisplayName, writeDisplayName } from "../components/UserMenu";
...
export function AppLayout() {
  ...
  useEffect(() => {
    const sync = () => {
      ...
      setMockUserId(localStorage.getItem("easyjob_mock_user_id") ?? "");
    };
  ...
  function clearLocalSession() {
    localStorage.removeItem("easyjob_mock_user_id");
    writeDisplayName("");
    setMockUserId("");
    setDisplayName("");
  }
        ...
        <nav className="shrink-0 space-y-0.5 px-2 pb-1 pt-2" aria-label="Primary">
          ...
          <details className="group/refer">
            ...
            <div className="border-l border-slate-200 py-1.5 pl-3 pr-1 text-[11px] leading-relaxed text-slate-600">
              Referrals and credits are <span className="font-semibold text-slate-800">not in EasyJob 1.0</span>. Share the repo with friends, or check back later.
            </div>
          </details>
        </nav>
