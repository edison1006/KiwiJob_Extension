#!/usr/bin/env bash
# Create PostgreSQL role + database for EasyJob (local dev).
...
try() {
  ...
  if psql -d postgres -v ON_ERROR_STOP=1 -f "$SQL"; then
    echo ""
    echo "Done. Set in easyjob/apps/api/.env:"
    echo "  DATABASE_URL=postgresql+psycopg2://easyjob:easyjob@${PGHOST}:${PGPORT}/easyjob"
    exit 0
  fi
  return 1
}
